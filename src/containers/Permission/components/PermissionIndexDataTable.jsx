...
const DataTable = ( {organization} ) => {
    ...
    //     e.preventDefault()
    //     setLoading( true )
    //     axios.delete(`/organization/${organization.id}/permission/${id}`)
    //     .then( (res) => {
    //         // console.log(res)
    //         if(res.status == 200)  {
    ...
    //         <>
    //             <span>
    //                 <Link to={`/permissions/view/${row.original.id}`}>View </Link>
    //                 <span style={{width:'15px', display: 'inline-block'}}></span>
    //                 <Link disabled={loading} to={'#delete-permission'} className="text-danger" onClick={(e) => {if(window.confirm('Are you sure to delete this permission?')){onClickDelete(e, row.original.id)}}}>Delete</Link>
    //             </span>
    //         </>
    //     )
    // }
    ...
    const apiUrl = `/organization/${organization.id}/permission`

    const { isLoading, error, data, isSuccess } = useQuery(
        ['permissions', apiUrl, queryPageIndex, queryPageSize, queryPageFilter, queryPageSortBy],
        () => fetchApiData(
            {
                url: apiUrl,
                page: queryPageIndex,
                    ...
                    <form className="form form--horizontal">
                        <div className="form__form-group pb-4">
                            <div className="col-md-9 col-lg-9">
                                <TableFilter onClickFilterCallback={onClickFilterCallback} label={'permissions'} />
                            </div>
