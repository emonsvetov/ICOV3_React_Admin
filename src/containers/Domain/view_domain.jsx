...
const ViewDomain = () => {
  ...
  const fetchDomain = async (id, organization) => {
    ...
    try {
      ...
      const response = await axios.get(
        `/organization/${organization.id}/domain/${id}`
      );
  ...
  useEffect(() => {
    fetchDomain(id, organization).then((response) => {
      setDomain(response);
      setIsLoading(false);
    });
  }, [id, organization]);
  ...
  if (domain) {
    return (
      <Container className="dashboard">
        ...
        {selected === "details" && (
          <DomainDetails organization={organization} data={domain} />
        )}
        {selected === "domain-programs" && (
          <DomainPrograms organization={organization} domain={domain} />
        )}
      </Container>
    );
  }
