...
const SKIP = [{ label: "1-800-Flowers", value: "1" }];
...
const DIRECT_ANSCESTORS = [{ label: "NONE", value: "none" }];
...
const AddSubmerchantModal = ({
  ...
  const fetchSubMerchants = async () => {
    try {
      ...
      const response = await axios.get(
        `/merchant/${merchant.id}/submerchant?minimal=true`
      );
      // console.log(response);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetchSubMerchants().then((response) => {
  ...
  const onSubmit = (values) => {
    setLoading(true);
    axios
      .post(`/merchant/${merchant.id}/submerchant`, {
        merchant_id: values.merchant.value,
        anscestor_id: values.anscestor?.value,
      })
      .then((res) => {
        // console.log(res)
        if (res.status == 200) {
              ...
              <Row className="w100">
                ...
                <Col md="6" lg="6" xl="6" className="text-right">
                  <ButtonToolbar className="modal__footer flex justify-content-right w100">
                    ...
                    <Button
                      type="submit"
                      disabled={loadingMerchants || loadingSubMerchants}
            ...
            <ModalBody className="modal-lg">
                ...
                <Col md="6">
                  <div className="form__form-group">
                    <span className="form__form-group-label">
                      Select from the dropdown list to add as a submerchant{" "}
                    </span>
              ...
              <Row className="w100">
                <Col md="6">
                  <div className="form__form-group">
                    <span className="form__form-group-label">
                      Select from the dropdown list to add as a Direct Anscestor
                    </span>
                    <div className="form__form-group-field position-relative">
                      <Field
                        name="anscestor"
