...
const DataTable = ({ merchant }) => {
  ...
  const onDeleteSubMerchant = (e, submerchant_id, dt = false) => {
    ...
    axios
      .delete(
        `/merchant/${merchant.id}/submerchant/${submerchant_id}/?dt=${dt}`
      )
      .then((res) => {
        // console.log(res)
        if (res.status == 200) {
      ...
      <>
        <Link
          ...
          onClick={(e) => {
            if (
              window.confirm(
                "Are you sure to delete this sub merchant with all its sub merchants?"
              )
            ) {
              onDeleteSubMerchant(e, row.original.id, true);
            }
          }}
        >
          Delete sub merchant and sub tree
        </Link>{" "}
        |
        <Link
          to={{}}
          disabled={loading}
          onClick={(e) => {
            if (window.confirm("Are you sure to delete this sub merchant?")) {
              onDeleteSubMerchant(e, row.original.id);
            }
          }}
        >
          Delete node
