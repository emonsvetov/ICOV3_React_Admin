...
const DataTable = ({ organization, program }) => {
  ...
  const [subprogram, setSubprogram] = useState(null) //used in change status modal
  ...
  columnns.forEach( (column, i) => {
    if( column.Header === 'Status')
    {
      columnns[i].Cell =  ({ row, value }) => { return strShowUserStatus(row.original)}
    }
})
  ...
  const apiUrl = `/organization/${program.organization_id}/program/${program.id}/subprogram`
    ...
    return (
      <>
        <div className='table react-table'>
          ...
          {subprogram && <ChangeStatusModal isOpen={isChangeStatusOpen} setOpen={setChangeStatusOpen} toggle={toggleChangeStatus} setTrigger={setTrigger} program={subprogram} />}
        </div>
        {(rows.length > 0) && (
          <>
