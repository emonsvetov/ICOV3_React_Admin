...
const AddProgramUserForm = ({ organization, program, toggle, setTrigger }) => {
  ...
  const getRoles = (organizationId) => {
    setSaving(true);
    fetchRoles(organizationId, true).then((data) => {
  ...
  const onSubmit = (values) => {
    ...
    axios
      .post(
        `/organization/${program.organization_id}/program/${program.id}/user`,
        values
      )
      .then((res) => {
        // console.log(res)
        if (res.status == 200) {
          ...
          dispatch(
            sendFlashMessage("User added successfully!", "alert-success")
          );
          setSaving(false);
        }
      })
