...
import DeletedSocialWallPostsFilter from "./DeletedSocialWallPostsFilter";
...
const DataTable = ({organization, programs}) => {
    ...
    const apiUrl = `/organization/${organization.id}/report/deleted-social-wall-posts`;
    const {isLoading, error, data, isSuccess} = useQuery(
        [['DeletedSocialWallPostsFilter', programId], apiUrl, queryPageIndex, queryPageSize, queryPageFilter, queryPageSortBy, queryTrigger],
        () => fetchApiData(
            {
                url: apiUrl,
                page: queryPageIndex,
                    ...
                    }
                    {
                        ...
                        <table {...getTableProps()} className="table table-striped report-table">
                            ...
                            {page.map(row => {
                                ...
                                let rowSpan = countSubRows ? countSubRows + 1 : 1;
                                return (
                                    <>
                                        <tr {...row.getRowProps()} key={row.id}>
                                            {
                                                row.cells.map(cell => {
                                                    ...
                                                    if (skip) return null;
                                                    if (cell.column.enableRowSpan === 1) {
                                                        rowSpan = 1;
                                                    } else {
                                                        rowSpan = countSubRows + 1;
                                                    }
                                                ...
                                                <tr {...subRow.getRowProps()} key={subRow.id}>
                                                    {
                                                        subRow.cells.map(subCell => {
                                                            ...
                                                            if (subCell.column.enableRowSpan !== 1) {
                                                                return null;
                                                            }
                                                            if (skip) return null;
