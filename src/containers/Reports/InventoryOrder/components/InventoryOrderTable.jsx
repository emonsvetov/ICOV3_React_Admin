...
const fetchMockData = () => {
    const newData = [];
    MOCK_DATA.forEach(merchantObj => {
        merchantObj.denomination.forEach( (item, index) => {
        newData.push({
            merchant: merchantObj.merchant,
            denomination: merchantObj.denomination[index],
            num_in_inventory: merchantObj.num_in_inventory[index],
            two_week_target: merchantObj.two_week_target[index],
        });
        });
    });

    const data = {
        results: renameChildrenToSubrows(newData),
        count: 15
    };
    return data;
};
                                    ...
                                    <tr {...row.getRowProps()}>
                                        {
                                            row.cells.map( cell => {
                                                if (cell.isRowSpanned){
                                                    ...
                                                    return <td rowSpan={cell.rowSpan} {...cell.getCellProps()} ><span style={cell.column.Header==='#' ? {paddingLeft: `${paddingLeft}px`} : null}>{cell.render('Cell')}</span></td>
                                                }
                                            })
                                        }
                                    </tr>
